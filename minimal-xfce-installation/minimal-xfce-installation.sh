#!/bin/bash
################################################################################
# This script install the minimal Xfce environment with NO extra package.      #
...
# Install X.Org & Xfce
if ! dnf install --assumeyes @xfce xdg-user-dirs xev xprop
then
...
# Enable Xfce
if ! systemctl enable lightdm graphical.target
then
    echo "Error while enabling LightDM."
    exit
fi
